                 ...
                 const std::function<void(const Error&)>& onError)
{
    ...
    const auto close_current_attrib = [&](){
        if (!entry.key.isEmpty()) {
            if (entry.values.empty()) {
                onError({ entry.line, LOGLIT("attribute value missing, entry ignored.") });
            }
            else
    ...
    while (stream.readLineInto(&line)) {
        ...
        if (line.at(0).isSpace()) {
            if (entry.key.isEmpty()) {
                onError({ linenum, LOGLIT("line starts with whitespace, but no attribute has been defined yet.") });
                continue;
            }
        ...
        // invalid line
        onError({ linenum, LOGLIT("line invalid, skipped.") });
    }
